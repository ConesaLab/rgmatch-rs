...
fn test_golden_output_match() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Point to ../data relative to rgmatch-rs/
    let full_data_dir = base_dir.parent().unwrap().join("data");
    ...
    let output_path = base_dir.join("tests").join("data").join("integration_test_output.txt");

    // Ensure test data exists
    if !gtf_path.exists() || !bed_path.exists() {
        eprintln!("Test data not found at {:?}. Skipping integration test.", full_data_dir);
        return Ok(());
    }
    ...
    let golden_file = File::open(&golden_path).or_else(|_| {
         eprintln!("Golden output not found at {:?}. Cannot verify correctness.", golden_path);
         // If gold missing, we can't verify, but maybe we shouldn't fail if user just wanted to run binary?
         // But this is a test.
         Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Golden file missing"))
    })?;
    ...
    if out_len != gold_len {
        ...
        panic!("File sizes differ! Output: {} bytes, Golden: {} bytes. (Content upstream matched)", out_len, gold_len);
    }
